...
class Tab extends React.Component {
  constructor(props) {
    ...
    this.botJoinMeeting = this.botJoinMeeting.bind(this);
  ...
  /**
   * Fetch the meeting's joinURL from the bot API
   */
   getMeetingInfo = async () => {
    ...
    // Request must be sent from the BotFramework service, so send it to the bot to make the Bot Framework REST request
    let serverURL = `${process.env.REACT_APP_TEAMSFX_BACKEND_ENDPOINT}/getMeetingJoinURL?conversationId=${chatId}&meetingId=${meetingId}`;
    ...
    if (!response.ok) {
      ...
      this.setState({ joinURL: data.joinUrl });
    }

  }

 /**
   * Call Voice Echo Bot API to join the current meeting
   */
  botJoinMeeting = async () => {

    // Request must be sent from the BotFramework service, so send it to the bot to make the Bot Framework REST request
    let serverURL = `${process.env.REACT_APP_TEAMSFX_BACKEND_ENDPOINT}/botJoinMeeting?joinURL=${this.state.joinURL}`;
   ...
   */
   botLeaveMeeting = async () => {

    let callId = this.state.callId;
    let graphPhotoEndpoint = `https://graph.microsoft.com/v1.0/communications/calls/${callId}`;
    let graphRequestParams = {
      method: 'DELETE',
      headers: {
        "authorization": "bearer " + this.state.graphAccessToken
  ...
  render() {
    ...
    let title = Object.keys(this.state.context).length > 0 ?
      `Welcome to the meeting ${this.state.displayName ? this.state.displayName : this.state.context['upn']}` : <Loader />;
      ...
      content =
        <div>
          ...
          <MediaQuery maxWidth={280}>
            ...
            {this.state.botJoined ? 
              <Button onClick={this.botLeaveMeeting} primary>Click to remove Echo Bot</Button> :
              <Button onClick={this.botJoinMeeting} primary>Click to call Echo Bot</Button>
            }
          </MediaQuery>
